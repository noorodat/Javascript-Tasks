...
function stringItUp(arr){
  let str = arr.map((ele) => ele.toString());
  return str;
}
console.log(stringItUp([2, 5, 100]));
console.log("----------------------------");
...
function readyToPutInTheDOM(arr){
  let info = arr.map((ele) => `<h1>${ele.name}</h1><h2>${ele.age}</h2>`);
  return info;
}
...
function doubleValues(arr){
  ...
  */
  ...
  function valTimesIndex(arr){
     let newArr = arr.map((ele, indx) => ele * indx);
    return newArr;
  }
